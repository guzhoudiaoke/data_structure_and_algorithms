...
class BST():
    ...
    def is_bst2(self):
        def check(node, min_node, max_node):
            if node is None:
                return True
            if min_node and min_node.key >= node.key:
                return False
            if max_node and max_node.key <= node.key:
                return False

            if not check(node.left, min_node, node):
                return False
            if not check(node.right, node, max_node):
                return False
    ...
    def is_bst(self):
        def check(node):
            ...
            if prev[0] and prev[0].key >= node.key:
                return False
...
def test_is_bst(count, maxkey):
    ...
    if bst.root and bst.root.left:
        p = bst.root.left
        while p.right:
            p = p.right
        p.key = bst.root.key+1
    print(bst.is_bst())
...
def test_is_cbt():
    ...
    bst.root = TreeNode(1, 1)
    print(bst.is_cbt())

    bst.root.left = TreeNode(2, 2)
    print(bst.is_cbt())

    bst.root.right = TreeNode(3, 3)
    print(bst.is_cbt())

    bst.root.left.left = TreeNode(4, 4)
    print(bst.is_cbt())

    bst.root.right.left = TreeNode(5, 5)
    print(bst.is_cbt())
